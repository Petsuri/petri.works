...
const findSubscription = async (client: DynamoDbClient, email: EmailAddress): Promise<Result<Option<Subscription>, string>> => {
  ...
  const result = await client.scanItems({ TableName: TableName, FilterExpression: "email = :email", ExpressionAttributeValues: { ":email": email.email } });
  if (!result.ok) {
...
const saveSubscription = async (client: DynamoDbClient, subscription: Subscription): Promise<Result<Unit, string>> => {
  const result = await client.put({ TableName: TableName, Item: { email: subscription.email.email, name: subscription.name.name } });
  if (result.ok) {
    return success(unit());
  }
  return failure(result.error.message);
